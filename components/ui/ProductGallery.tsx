...
export default function ProductGallery({ images, alt, priority = false }: ProductGalleryProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* Main image */}
      <div
        className="relative aspect-square rounded-2xl overflow-hidden bg-brand-cream border border-brand-border shadow-sm select-none"
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide justify-center">
          {images.map((src, i) => (
            <button
              key={i}
              onClick={() => setActive(i)}
              className={`relative w-14 h-14 rounded-xl overflow-hidden border-2 shrink-0 transition-all duration-200 ${
                i === active
